/**
 * Pindown PinDOWN implementation
 */
...
char *get_inode_policy(struct inode *inode, const char *name)
{
    ...
    //printk("get_inode_policy: inode_no = %lu\n", inode->i_ino);
...
int pindown_inode_permission(struct inode *inode, int mask, struct nameidata *nd)
{
    ...
    sec = (pindown_security_t *)current->security;

    /* Get the inode policy */
    inode_policy = get_inode_policy(inode, "security.pindown");
    if (inode_policy == NULL) {
        //printk("pindown_inode_permission: No inode policy set, grant access\n");
        rc = 0;
        goto out;
    }

    /* Compare process security info to inode policy */
    if (sec != NULL && (strcmp(inode_policy, sec->bprm_pathname) == 0)) {
        //printk("pindown_inode_permission: Match, grant access\n");
        rc = 0;
    }
...
out:
    ...
    /*if (rc == -EACCES) {
        if (sec != NULL)
        printk("pindown_inode_permission: deny access to inode with sec=%s p=%s\n", sec->bprm_pathname, inode_policy);
    }*/
    return rc;
}
...
int pindown_task_alloc_security(struct task_struct * p)
{
    ...
    if (sec == NULL) {
        //printk("pindown_task_alloc_security: kmalloc failed\n");
        err = -ENOMEM;
        goto out;
    } 
    ...
    //if called from pindown_bprm_set_security()
    if(current == p) {
        goto out;
    }
    
    //if called because of fork()
    parent_sec = (pindown_security_t *)current->security;
    if (parent_sec != NULL) {
        sec->pathlen = parent_sec->pathlen;
        strcpy(sec->bprm_pathname, parent_sec->bprm_pathname);
        //printk("pathlen = %u\t pathname=%s\n", sec->pathlen, sec->bprm_pathname);
    }
...
int pindown_bprm_set_security(struct linux_binprm * bprm)
{
    ...
    if (!rc) {
        ...
        if (bprm != NULL && bprm->filename != NULL) {
            u32 len = strlen(bprm->filename);
            if (len >= MAX_PATHLEN) {
                //printk("pindown_bprm_set_security: Too long binary filename\n");
                rc = -ENOMEM;
            }
            else {
